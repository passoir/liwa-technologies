 ...
 * </ul>
 */
public class XmiTimesYearsWriterCasConsumer extends CasConsumer_ImplBase {
	/**
	...
	public void processCas(CAS aCAS) throws ResourceProcessException {
		...
		if (it.hasNext()) {
			...
			try {
				...
				String yearPart = inFile.getParent();
				...
				outFileName += ".xmi";
				String outFolder = (String) getConfigParameterValue(PARAM_OUTPUTDIR);
				// + yearPart;

				File mnewOutputDir = new File(outFolder);
				// if (!mnewOutputDir.exists()) {
				// mnewOutputDir.mkdirs();
				// }
				// outFile = new File(mOutputDir, outFileName);
				// commented out
				// outFile = new File(mnewOutputDir, outFileName);

				// modelFileName = mOutputDir.getAbsolutePath() + "/" +
				// inFile.getName() + ".ecore";

				// commented out
				// modelFileName = mnewOutputDir.getAbsolutePath() + "/"
				// + inFile.getName() + ".ecore";
		...
		/*  try { writeXmi(jcas.getCas(), outFile, modelFileName); } catch
		  (IOException e) { throw new ResourceProcessException(e); } catch
		  (SAXException e) { throw new ResourceProcessException(e); }
		 */
