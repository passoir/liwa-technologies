 ...
 * 
 */
public class AnnotationsToDBAnnotator extends JCasAnnotator_ImplBase {
	 ...
	 * 
	 */
	public AnnotationsToDBAnnotator() {
	...
	//	System.out.println("started :" + filename);
		...
		//	Hashtable<String, Integer> lemmasHash = LiwaDatabase.insertArcLemmas(lemmas);
			LiwaDatabase.insertArcLemmasSimple(lemmas);
				...
				if (insertedSentences.containsKey(sentence.getBegin())) {
					...
					while (tokenIterator.hasNext()) {
						...
						if (PosNoun.contains(token_pos)) {
							...
							if((conjEnd.contains(begin)|| conjEnd.contains(begin-1)||conjEnd.contains(begin-2))|| (conjStart.contains(token.getEnd())||conjStart.contains(token.getEnd()+1)||conjStart.contains(token.getEnd()+2))){
			...
			if (tokenstrings.size() > 0) {
				LiwaDatabase.insertArcDocumentLemmasNoIds(tokenstrings);
			}
			if (conjuctionstrings.size() > 0) {
				LiwaDatabase.insertArcDocumentConj(conjuctionstrings);
			}
			//updating the document_lemma table, ids instead of lemmas, at the end of the archive.
			if(isLastFile){
				
				LiwaDatabase.lastDocumentCleanul();
			}
