...
public class PathSharingContext extends FileSystemXmlApplicationContext {
	private static Logger LOGGER = Logger.getLogger(PathSharingContext.class
			.getName());
	...
	public PathSharingContext(String[] configLocations,
			ApplicationContext parent) throws BeansException {
		super(configLocations, parent);
		// TODO Auto-generated constructor stub
	}

	public PathSharingContext(String[] configLocations, boolean refresh,
			ApplicationContext parent) throws BeansException {
	...
	public void start() {
		...
		for (Iterator it = new HashSet(lifecycleBeans.keySet()).iterator(); it
				.hasNext();) {
	...
	protected void doStart(Map lifecycleBeans, String beanName) {
		...
		if (bean != null) {
			String[] dependenciesForBean = getBeanFactory()
	...
	protected void doStop(Map lifecycleBeans, String beanName) {
		Lifecycle bean = (Lifecycle) lifecycleBeans.remove(beanName);
		if (bean != null) {
			String[] dependentBeans = getBeanFactory().getDependentBeans(
					beanName);
	...
	protected Map getLifecycleBeans() {
		...
		String[] beanNames = beanFactory.getBeanNamesForType(Lifecycle.class,
				false, false);
	...
	protected void doClose() {
		if (isActive()) {
			...
			// destruction.
			Map lifecycleBeans = getLifecycleBeans();
			for (Iterator it = new LinkedHashSet(lifecycleBeans.keySet())
					.iterator(); it.hasNext();) {
				String beanName = (String) it.next();
				doStop(lifecycleBeans, beanName);
			}
