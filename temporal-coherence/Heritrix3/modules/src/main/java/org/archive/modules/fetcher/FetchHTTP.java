 ...
 * @contributor stack
 * @contributor others
 * @version $Id: FetchHTTP.java 6647 2009-11-12 01:28:22Z nlevitt $
 */
public class FetchHTTP extends Processor implements Lifecycle {
	...
	private static Map<String, Object> monitors = new HashMap<String, Object>();
	...
	protected void innerProcess(final CrawlURI curi)
		...
		try {
			host = curi.getBaseURI().getHost();
			synchronized (monitors) {
				monitor = monitors.get(host);
				if (monitor == null) {
					monitor = new Object();
		...
		if (digestContent) {
			curi.setContentDigest(algorithm, rec.getRecordedInput()
					.getDigestValue());
		}
	...
	/**
	 * Update CrawlURI internal sizes based on current transaction (and in the
	 * case of 304s, history)
	 * 
	 * @param curi
	 *            CrawlURI
	 * @param rec
	...
	protected void setSizes(CrawlURI curi, Recorder rec) {
		...
		if (curi.getFetchStatus() == HttpStatus.SC_NOT_MODIFIED
			...
			if (history[0] != null
				...
				long referenceLength = (Long) history[0]
						.get(A_REFERENCE_LENGTH);
	...
	private void setCharacterEncoding(CrawlURI uri, final Recorder rec,
		...
		try {
			...
			uri.getAnnotations().add(
					"charset_err:" + StringUtils.stripToEmpty(encoding));
			encoding = null;
		}
		if (encoding == null) {
	...
	protected HostConfiguration configureMethod(CrawlURI curi, HttpMethod method) {
		...
		if (getSendReferer()
				&& !LinkContext.PREREQ_MISC.equals(curi.getViaContext())) {
		...
		HostConfiguration config = new HostConfiguration(http
	...
	/**
	 * Set the given conditional-GET header, if the setting is enabled and a
	 * suitable value is available in the URI history.
	 * 
	 * @param curi
	 *            source CrawlURI
	 * @param method
	 *            HTTP operation pending
	 * @param setting
	 *            true/false enablement setting name to consult
	 * @param sourceHeader
	 *            header to consult in URI history
	 * @param targetHeader
	 *            header to set if possible
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Setup proxy, based on attributes in CrawlURI and settings, in given
	 * HostConfiguration
	 */
	...
	private void configureBindAddress(CrawlURI curi, HostConfiguration config) {
		String addressString = (String) getAttributeEither(curi,
				HTTP_BIND_ADDRESS);
	...
	/**
	 * Get a value either from inside the CrawlURI instance, or from settings
	 * (module attributes).
	 * 
	 * @param curi
	 *            CrawlURI to consult
	...
	protected void handle401(final HttpMethod method, final CrawlURI curi) {
		...
		HttpAuthenticationCredential extant = HttpAuthenticationCredential
		...
		if (extant != null) {
			...
			CrawlServer server = serverCache.getServerFor(serverKey);
			Set<Credential> storeRfc2617Credentials = getCredentialStore()
					.subset(curi, HttpAuthenticationCredential.class,
							server.getName());
	...
	private Set<Credential> getCredentials(CrawlURI curi, Class<?> type) {
		...
		if (curi.hasCredentialAvatars()) {
			for (Iterator<CredentialAvatar> i = curi.getCredentialAvatars()
					.iterator(); i.hasNext();) {
