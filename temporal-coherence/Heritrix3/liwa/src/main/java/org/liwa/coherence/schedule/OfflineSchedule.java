...
public abstract class OfflineSchedule extends AbstractSchedule {
	...
	protected void computeExpected() {
		...
		for (int i = -(size - 1); i <= 0; i++) {
			...
			expected += c.getPriority()
					* Math.pow(Math.E, -c.getChangeRate()
	 ...
	 * 
	 * @param visit
	 *            current visit, negative number
	 * @return a position object which containts the page which must be
	 *         scheduled at the position visit and as well as its revisit
	 *         position
	 */
	...
	protected void printSchedule() {
		...
		for (int i = -(size - 1); i <= 0; i++) {
			SchedulablePage c = visitMap.get(i);
			System.out.println(timeMapper.getInterval(c
					.getIntLength(confidenceThreshold))
					+ " "
